...
def save_list_patient_record(record: Record, patients, prediction_results) -> int:
    ...
    result = db.insert_patient(list_of_patients)
...
def make_one_prediction(record: Record, patient: Patient) -> dict:
    """
        _summary_: Make a prediction for a single patient 
    :param record: (Record): _description_
...
def make_multiple_prediction(record: Record, patients: List[Patient]):
    ...
    :param record: (Record): _description_
    :param patients: (List[Patient]): _description_
    :return: _type_: description
    """

    records = [dict(patients[patientindex]) for patientindex in range(len(patients))]
    ...
    save_list_patient_record(record, patients, results)
...
async def get_patients_by_window_period(fromdate:str, todate: str) -> List[Patient]:
    ...
    patients = db.get_patient_by_full_name(from_to_dict)
...
async def get_patient_by_file_name(filename: str, createdon: str):
    ...
    patients = db.get_patient_by_full_name(filename, year, month, day)
